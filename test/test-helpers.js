...
const TestHelpers = {
  seedDbTables(db, projects, categories, tasks) {
    return db.transaction(async (trx) => {
      ...
      await trx.raw(`SELECT setval('wedo_projects_id_seq', ?)`, [
        projects[projects.length - 1].id,
      ]);

      if (categories) {
        await trx.into("wedo_categories").insert(categories);
        await trx.raw(`SELECT setval('wedo_categories_id_seq', ?)`, [
      ...
      if (tasks) {
        ...
        await trx.raw(`SELECT setval('wedo_tasks_id_seq', ?)`, [
      ...
      trx
        ...
        .then(() =>
          Promise.all([
            ...
            trx.raw(
              `ALTER SEQUENCE wedo_categories_id_seq minvalue 0 START WITH 1`
            ),
            trx.raw(
              `ALTER SEQUENCE wedo_projects_id_seq minvalue 0 START WITH 1`
            ),
