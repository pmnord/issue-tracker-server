...
describe("Task endpoints", () => {
  ...
  afterEach("Clean the database tables again", () =>
    TestHelpers.truncateDbTables(db)
  );

  /* ---------------------------------- Tests --------------------------------- */
  describe(`POST /task`, () => {
    beforeEach("Seed the database", () =>
      TestHelpers.seedDbTables(db, testProject, [testCategories[0]])
    );
      ...
      return supertest(app)
        ...
        .then(() => {
          return supertest(app)
            ...
            .expect((response) =>
              assert.equal(
                response.body.categories[0].tasks[0].title,
                testTasks[0].title
              )
            );
        });
  ...
  describe(`PATCH /task/:task_uuid`, () => {
    beforeEach("Seed the database", () =>
      TestHelpers.seedDbTables(
        db,
        testProject,
        [testCategories[0]],
        [testTasks[0]]
    ...
    it(`Returns 204 and updates the task`, () => {
      const newTaskValues = {
        title: "Updated title",
        category_uuid: '1',
        tags: '',
      ...
      return supertest(app)
        .patch(`/api/task/${testTasks[0].uuid}`)
        .set("api-key", config.WEDO_API_KEY)
        .send(newTaskValues)
        .expect(204)
        .then(() => {
  ...
  describe(`DELETE /task/:task_id`, () => {
    ...
    it(`Returns 201 and posts a new task`, () => {
      return supertest(app)
        .delete(`/api/task/${testTasks[0].uuid}`)
        .set("api-key", config.WEDO_API_KEY)
        ...
        .then(() => {
          return supertest(app)
            ...
            .expect(200)
            .expect((response) =>
              assert.equal(response.body.categories[0].tasks.length, 0)
            );
        });
    });
  });
